...
examples:
- "How has the project progressed from start to now?"
- "How did our architecture evolve?"
- "What changes have happened over the past three months?"
---
...
- Search recorded memory for timeline-related records:
  memory_search({ query: "<topic_keywords> progress evolution changes", corpus: "memory", maxResults: 10, minScore: 0.1 })
...
- Identify the **tracking topic** from the user's question:
  - Project progress: "PCA exam prep progress?" → topic = `PCA exam prep mock exam progress`
  - Architecture evolution: "How did our architecture evolve?" → topic = `architecture evolution design`
  - Health changes: "Weight changes over 3 months?" → topic = `weight body fat InBody`
- Extract CJK keywords (nouns separated by spaces), add time-related words like "progress," "evolution," "changes."

**Boundary condition detection** (SOP boundary condition 5 — Long-Term Path Discovery):
...
**Time aggregation mode** (SOP boundary condition 5 — when time span > 30 days):
```javascript
// Start segment search
memory_search({
  query: "<topic_keywords> start initial earliest",
  corpus: "memory",
  maxResults: 10,
  minScore: 0.1
})
...
memory_search({
  query: "<topic_keywords> middle progress evolution",
  corpus: "memory",
  maxResults: 10,
  minScore: 0.1
})
...
// End segment search
memory_search({
  query: "<topic_keywords> complete result final latest",
  corpus: "memory",
  maxResults: 10,
  minScore: 0.1
})
